...
static void MPU_Config(void)
{
  ...
  // Also define a second region for external SDRAM - Write Through, No Write Allocate
  // Struct reused but all fields filled
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
  ...
  MPU_InitStruct.Number = MPU_REGION_NUMBER1;                 // New region number
  MPU_InitStruct.TypeExtField = MPU_TEX_LEVEL0;
  MPU_InitStruct.SubRegionDisable = 0x00;
  MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_ENABLE;
...
void stm32f7_wm8994_init(uint32_t fs, int16_t io_method, 
  int16_t select_input, int16_t select_output, 
  int16_t headphone_gain, int16_t line_in_gain, int16_t dmic_gain, 
  char * name,
  int graph)
{
  ...
  init_LCD(fs, name, io_method, graph);
	wm8994_Reset(AUDIO_I2C_ADDRESS);  // perform a hard reset; added 7/16/24	
  switch(io_method)
  {
    case IO_METHOD_DMA:
   ...
   case IO_METHOD_INTR:
      ...
      BSP_AUDIO_SAI_INTERRUPT_INIT(&rx_sample_L, &rx_sample_R, &tx_sample_R, &tx_sample_L);  // swapped tx_sample_R, tx_sample_L 7/16/24
      break;
